...
const ExperiencesCard = ({ year, position, company, count }) => {
  return (
    <div
      className="mb-8 flex flex-col md:flex-row lg:flex-row justify-center items-center w-full"
      data-aos="zoom-in">
      <div className="flex flex-row">
        <div className="z-20 w-6 h-6 rounded-full bg-color2 overflow-visible shadow mx-1">
          {/* {" "}
          {count < 1 && (
            <div class="hidden lg:block absolute w-[0.1rem] bg-black h-auto lg:h-full ms-[0.6rem]"></div>
          )} */}
        </div>
        <div className="flex mx-1">
          <h1 className="bg-color3 rounded-md px-2 py-1 text-sm text-white">
            {year}
          </h1>
        </div>
      </div>

      <div className="bg-color2 rounded-lg shadow-xl w-full md:w-6/12 px-6 py-4 mt-5 md:mt-0 mx-1">
